...
FIXED (per P3's review): every profile now generates independent traffic for EVERY
account it's given, not just one picked at random. If a scenario YAML asks for 5
casual accounts, all 5 should actually send traffic — previously casual/batch/bursty
silently used only one, wasting the rest.
"""
...
def casual(rng: random.Random, accounts: list[dict], duration_s: int = 3600,
    ...
    """Ordinary accounts, each asking at a slow constant rate all day. The baseline case."""
...
def batch(rng: random.Random, accounts: list[dict], duration_s: int = 3600,
          burst_size: int = 400):
    """
    Each account given is one overnight batch job: silence, then one big tight burst,
    then silence again. High volume from a predictable account doing a predictable
    thing — should NOT be confused with an attacker. (This is why SCHEMA.md's `tier`
    column exists — an enterprise batch customer at high rate isn't automatically a
    suspect.) One account = one job; give it more accounts to model several jobs.
    """
    for account in accounts:
...
def bursty(rng: random.Random, accounts: list[dict], duration_s: int = 3600,
    ...
    """
    Each account given is a normal app with flaky networking: sometimes it re-sends
    the same request 2-3 times within a couple seconds because it didn't see the
    first response in time. Different from `batch` — small frequent bursts, not one
    big overnight one.
    """
    for account in accounts:
        t = 0.0
